...
bool LiteAudioEngine::Initialize(AudioCodec* codec, int frame_duration_ms) {
    codec_ = codec;
    frame_samples_ = frame_duration_ms * 16000 / 1000;
    output_buffer_.reserve(frame_samples_);

    // esp-sr was removed from this board's build. With the standalone WakeNet
    // compiled in (older configuration) the engine still creates the wake
    // word object; otherwise it stays null and detection is a no-op.
#if CONFIG_USE_ESP_WAKE_WORD
    should_have_wake_word_ = true;
#endif
    {
...
bool LiteAudioEngine::CreateWakeWordLocked() {
    ...
    if (!wake_word->Initialize(codec_)) {
        ESP_LOGE(TAG, "Failed to initialize standalone WakeNet");
        return false;
    }
    wake_word_ = std::move(wake_word);
#else
    // esp-sr is not part of this build: no standalone WakeNet to create.
#endif
    return true;
}
