...
public:
    ...
    bool IsIdle();
    bool IsPlaybackIdle();
    bool WaitForPlaybackIdle(uint32_t timeout_ms);
    bool StopOutputIfIdle();
    ...
    // Posts the input-stop request so the codec input closes as soon as no
    // capture source (testing / wake word / voice processing) is active,
    // instead of waiting for the idle power timer.
