...
void AudioService::Start() {
    ...
    // Measured on-device (throttled high-water log): the CELT encoder peaks
    // at ~19.7 KiB of stack while speaking, so even 20 KiB left only 772 B
    // of margin and the original 12 KiB attempt fired the stack canary deep
    // in celt_encode_with_ec. Keep the original 24 KiB; the log guards any
    // future retuning attempt.
    xTaskCreate(
        [](void* arg) {
            AudioService* audio_service = (AudioService*)arg;
...
void AudioService::OpusCodecTask() {
    // Throttled stack high-water logging: the 12 KiB stack was chosen from
    // estimated encoder usage, so keep a cheap runtime probe until the
    // margin is confirmed on-device.
    auto last_stack_log = std::chrono::steady_clock::now() - std::chrono::seconds(30);
    while (true) {
        std::unique_lock<std::mutex> lock(audio_queue_mutex_);
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_stack_log).count() >= 30) {
            last_stack_log = now;
            ESP_LOGI(TAG, "opus_codec stack high water: %u bytes",
        ...
        if (!audio_decode_queue_.empty() &&
            ...
            try {
                    ...
                    if (ret == ESP_AUDIO_ERR_OK) {
                        ...
                        if (decoder_sample_rate_ != codec_->output_sample_rate() &&
                            ...
                            esp_ae_rate_cvt_get_max_out_sample_num(output_resampler_,
                                                                   task->pcm.size(), &target_size);
                            std::vector<int16_t> resampled(target_size);
                            uint32_t actual_output = target_size;
                            esp_ae_rate_cvt_process(
                                output_resampler_, (esp_ae_sample_t)task->pcm.data(),
                ...
                // Audio prompts are optional. Under transient TLS pressure,
                // drop this frame and keep the codec task alive instead of
                // allowing operator new to terminate the whole firmware.
                ESP_LOGE(TAG, "Dropping playback frame: out of memory");
            }
...
void AudioService::RequestInputStop() {
    // The idle power timer only closes the codec input after
    // AUDIO_POWER_TIMEOUT_MS. Memory-constrained phases (search/image on the
    // C3) need that headroom now: waking the input task lets it close the
    // input as soon as no capture source is active. Safe when a source is
    // still starting: the task rechecks the active bits after clearing the
    // request and skips the close in that case.
...
void AudioService::PlaySound(const std::string_view& ogg) {
    try {
        ...
        ESP_LOGE(TAG, "Dropping sound: out of memory");
        ResetDecoder();
    }
}
...
bool AudioService::WaitForPlaybackIdle(uint32_t timeout_ms) {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(timeout_ms), [this]() {
        return service_stopped_.load() || IsPlaybackDrainedLocked();
    });
    return IsPlaybackDrainedLocked();
}

bool AudioService::StopOutputIfIdle() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    ...
    // Keep TX clock while duplex input is active; stopping it can stall RX
    // on boards whose I2S channels share clocks.
    if (codec_->duplex() && codec_->input_enabled()) {
        return false;
    }
    codec_->EnableOutput(false);
    return !codec_->output_enabled();
}
...
bool AudioService::InitializeAudioEngine() {
    ...
    if (!audio_engine_->Initialize(codec_, OPUS_FRAME_DURATION_MS)) {
