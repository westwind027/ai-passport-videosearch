...
protected:
    ...
    std::chrono::system_clock::time_point last_clock_update_time_;
    esp_timer_handle_t notification_timer_ = nullptr;
    std::unique_ptr<DynamicGlyphCache> dynamic_glyph_cache_;
    // Retired text fonts stay alive for the lifetime of the display. LVGL
    // styles store raw lv_font_t pointers and custom pages (Wi-Fi picker,
    // keyboard, settings menu) bind the theme font at widget-creation time,
    // so freeing a replaced font would leave dangling pointers behind and
    // crash the render loop with an instruction access fault.
