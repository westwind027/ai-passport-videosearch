...
bool LvglDisplay::SetTextFont(std::shared_ptr<LvglFont> text_font) {
    ...
    // SetTheme() only rebinds the widgets it owns. Labels on custom pages
    // (Wi-Fi picker rows, keyboard, settings menu) keep the previous font's
    // raw pointer in their local styles, so the retired fonts must stay
    // alive; freeing them caused instruction-access-fault crashes whenever
    // Apply() ran a second time (boot-time apply + activation re-apply).
    // The font structs are small and their bitmaps live in the mmap'ed
    // assets partition, so retaining them is cheap.
    if (previous_light_font != nullptr) {
        retired_text_fonts_.push_back(std::move(previous_light_font));
    }
    if (previous_dark_font != nullptr) {
        retired_text_fonts_.push_back(std::move(previous_dark_font));
    }
    return true;
}
...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    // Update mute icon
    {
        DisplayLockGuard lock(this);
        if (mute_label_ != nullptr) {
            // Update icon if mute state changes. Displays which intentionally
            // omit this optional icon must still receive clock/network/battery
            // updates below.
    ...
    // Update time. LcdDisplay owns a dedicated top-left clock label; the
    // fallback keeps the original centered status behavior for other displays.
    const auto now = std::chrono::system_clock::now();
    if (time_label_ != nullptr) {
        if (last_clock_update_time_ + std::chrono::seconds(10) < now) {
            time_t current_time = time(NULL);
            struct tm* local_time = localtime(&current_time);
            if (local_time != nullptr && local_time->tm_year >= 2025 - 1900) {
                char time_str[16];
                strftime(time_str, sizeof(time_str), "%H:%M", local_time);
                DisplayLockGuard lock(this);
                lv_label_set_text(time_label_, time_str);
                last_clock_update_time_ = now;
            } else if (local_time != nullptr) {
                ESP_LOGW(TAG, "System time is not set, tm_year: %d", local_time->tm_year);
            }
        }
    } else if (app.GetDeviceState() == kDeviceStateIdle &&
               last_status_update_time_ + std::chrono::seconds(10) < now) {
