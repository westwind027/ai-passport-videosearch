...
docker build \
  ...
  -t folo/firmware-builder:idf55.5-arm64 .
```

The base image defaults to `espressif/idf:v5.5.5`, matching the supported local
toolchain and the repository CI build.
...
docker run --rm --platform linux/arm64 \
  -e FIRMWARE_BOARD_DIR=folo/ai-passport-c3 \
  -e FIRMWARE_BOARD_NAME=folo-ai-passport-c3 \
  -e FIRMWARE_LANGUAGE=zh-CN \
  -e FIRMWARE_WAKE_WORD=nihaoxiaozhi \
  -v "$PWD/output:/output" \
  folo/firmware-builder:idf55.5-arm64
```
