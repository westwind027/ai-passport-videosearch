...
def _wake_word_sdkconfig_options(
    ...
    # esp-sr was removed from the lite-target build (no local wake word on
    # the C3); historical commands that still pass a model keep working by
    # falling back to the disabled configuration.
    if target in _LITE_WAKE_WORD_TARGETS and normalized != "disabled":
        print(
            f"[WARN] esp-sr is not part of the {target} build; "
            f"treating --wake-word {wake_word!r} as 'disabled'.",
            file=sys.stderr,
        )
        return _wake_word_sdkconfig_options("disabled", target)
